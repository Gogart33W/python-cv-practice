...
class TrophyObserver:
    ...
    def __init__(self, brawler_list):
        ...
        # НОВА СИСТЕМА КУБКІВ (ПРАЙМИ) - ВТРАТА КУБКІВ
        self.trophy_lose_ranges = [
            (49, 0), (299, 1), (599, 2), (799, 3), (999, 4), 
            (1099, 5), (1199, 6), (1299, 7), (1499, 8), (1799, 9), 
            (3999, 10), (float("inf"), 15)
        ]
        
        # НОВА СИСТЕМА КУБКІВ (ПРАЙМИ) - ОТРИМАННЯ КУБКІВ
    ...
    def win_streak_gain(self):
        # Максимальний бонус за вінстрік тепер +10
        return max(0, min(self.win_streak - 1, 10))
    ...
    def find_game_result(self, screenshot, current_brawler, game_result=None):
        if not game_result:
            ...
            # ДОДАЛИ: Тепер бот чесно скаже, який мусор він бачить на екрані
            print(f"[OCR] Зчитано текст: '{text}' | Розпізнано як: {game_result} | Точність: {ratio}")

            # ЗНИЗИЛИ: Поріг до 0.20, щоб він зараховував катки навіть з кривим розпізнаванням
            if ratio < 0.20:
                if ratio > 0:
