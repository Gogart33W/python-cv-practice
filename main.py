...
# 🔥 ФІКС: Глушимо фантомні помилки Tkinter, щоб не засмічували консоль
class FilteredStderr:
    def __init__(self, original_stderr):
        self.original_stderr = original_stderr
        
    def write(self, msg):
        if "RuntimeError: main thread is not in main loop" in msg or "Exception ignored in" in msg or "tkinter" in msg:
            return
    ...
    # Додано метод fileno, щоб GUI PylaAI не крашився при перехопленні логів
    def fileno(self):
    ...
    class Main:
        ...
        def manage_time_tasks(self, frame):
            ...
            if self.Time_management.no_detections_check():
                # 🔥 ФІКС: Безпечно беремо словник з детекціями зі старого або нового play.py
                frame_data = getattr(self.Play, 'time_since_detections', getattr(self.Play, '_last_det', {}))
...
try:
    ...
    print("[SYSTEM] Init done. Launching GUI...")
